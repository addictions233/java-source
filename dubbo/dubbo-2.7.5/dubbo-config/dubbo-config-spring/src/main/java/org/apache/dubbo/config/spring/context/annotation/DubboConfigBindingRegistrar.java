...
public class DubboConfigBindingRegistrar implements ImportBeanDefinitionRegistrar, EnvironmentAware {
    ...
    public void registerBeanDefinitions(AnnotationMetadata importingClassMetadata, BeanDefinitionRegistry registry) {
        ...
        // 读取@EnableDubboConfigBinding注解属性值
...
//        dubbo.protocols.p2.host=0.0.0.0
        ...
        for (String beanName : beanNames) {
            ...
            // 注册一个DubboConfig的bean
            // 为每个beanName,注册一个空的BeanDefinition
            registerDubboConfigBean(beanName, configClass, registry);

            // 为每个bean注册一个DubboConfigBindingBeanPostProcessor的Bean后置处理器
            // 使用后置处理器为上一步生成的ApplicationConfig的Bean对象的属性进行赋值
            // fix: 不用每个配置对象都生成一个BeanPostProcessor进行赋值, 可以共用一个, 后续版本已经优化
    ...
    private void registerDubboConfigBean(String beanName, Class<? extends AbstractConfig> configClass,
        ...
        // ApplicationConfig对象, 继承自AbstractConfig

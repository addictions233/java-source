...
public class DefaultApplicationDeployer extends AbstractDeployer<ApplicationModel> implements ApplicationDeployer {
    ...
    public void initialize() {
        ...
        synchronized (startLock) {
            ...
            // 开启配置中心, ConfigCenterConfig
            startConfigCenter();

            // 加载应用配置: ApplicationConfig,ProtocolConfig,RegistryConfig,MetadataReportConfig
            ...
            // @since 2.7.8
            // 开启元数据中心, 由于应用级注册只用将ip和端口注册到注册中心
            // 而dubbo进行服务调用是具体到接口级别的, 需要在元数据中心暴露接口信息给调用方
            startMetadataCenter();

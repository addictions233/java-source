...
public interface DubboBeanUtils {
    ...
    static void registerCommonBeans(BeanDefinitionRegistry registry) {
        ...
        // 用于处理 @DubboReference 注解的 Bean 后置处理器。
        ...
        // 用于监听 Dubbo 部署和配置相关事件的应用监听器。
        // 重点: 注册DubboDeployApplicationListener, 在spring容器启动后进行服务导出和启动Netty等
        // 监听spring 容器启动完成事件
        registerInfrastructureBean(registry, DubboDeployApplicationListener.class.getName(), DubboDeployApplicationListener.class);
        // 注册DubboConfigApplicationListener, 初始化dubbo配置bean
        // 监听dubbo 配置初始化事件
        ...
        // 用于处理 Dubbo 配置默认属性值的 Bean 后置处理器。
        registerInfrastructureBean(registry, DubboConfigDefaultPropertyValueBeanPostProcessor.BEAN_NAME,
            DubboConfigDefaultPropertyValueBeanPostProcessor.class);

        // Dubbo config initializer
        // 用于初始化 Dubbo 配置 Bean 的初始化器。
        registerInfrastructureBean(registry, DubboConfigBeanInitializer.BEAN_NAME, DubboConfigBeanInitializer.class);

        // register infra bean if not exists later
        // 用于注册 Dubbo 基础设施 Bean 的后置处理器。

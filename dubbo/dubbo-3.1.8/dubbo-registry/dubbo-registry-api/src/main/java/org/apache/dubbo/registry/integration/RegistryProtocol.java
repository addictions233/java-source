...
public class RegistryProtocol implements Protocol, ScopeModelAware {
    ...
    private void register(Registry registry, URL registeredProviderUrl) {
        // 进行服务注册：分为应用级注册和接口级注册
        // 将 registeredProviderUrl 注册到注册中心
    ...
    public <T> Exporter<T> export(final Invoker<T> originInvoker) throws RpcException {
        ...
        //export invoker
        // 在导出本地服务，返回一个可变更的导出器包装器
        // 重点：启动并暴露本地服务，启动Netty/Jetty等
        ...
        if (register) {
            // 调用 register 方法将服务注册到注册中心。
            // 这里用两种情况，接口级注册会将接口级服务提供者数据直接注册到Zookeeper上
            // 服务发现（应用级注册）这里仅仅会将注册数据转换为服务元数据信息等后面再发布元数据
    ...
    private <T> ExporterChangeableWrapper<T> doLocalExport(final Invoker<T> originInvoker, URL providerUrl) {
        ...
        return (ExporterChangeableWrapper<T>) bounds.computeIfAbsent(key, s -> {
            ...
            // 调用 protocol.export 方法将服务暴露到本地。
            // 这里会根据协议类型，比如 Dubbo 协议，将服务暴露到 Dubbo 协议的服务端。

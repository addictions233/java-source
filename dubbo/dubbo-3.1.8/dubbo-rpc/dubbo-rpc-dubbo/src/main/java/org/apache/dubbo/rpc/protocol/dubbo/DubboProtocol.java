...
public class DubboProtocol extends AbstractProtocol {
    ...
    public <T> Exporter<T> export(Invoker<T> invoker) throws RpcException {
        checkDestroyed();
        // 从Invoker实例中获取服务的URL，该URL包含了服务的相关配置信息
        URL url = invoker.getUrl();

        // export service.
        String key = serviceKey(url);
        // 创建一个DubboExporter实例，用于管理导出的服务。
        // DubboExporter会将Invoker和服务的唯一标识关联起来，并将其存储在exporterMap中。
        ...
        // 开启服务，监听客户端的连接请求
        // 当请求过来后：形成服务key --> exportMap.get(Key) --> exporter --> invoker --> invoker.invoke(invocation)
        openServer(url);

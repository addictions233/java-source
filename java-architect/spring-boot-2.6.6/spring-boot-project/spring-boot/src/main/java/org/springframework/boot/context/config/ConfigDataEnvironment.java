...
class ConfigDataEnvironment {
	...
	ConfigDataEnvironment(DeferredLogFactory logFactory, ConfigurableBootstrapContext bootstrapContext,
		...
		// 从spring.factories中拿到ConfigDataLocationResolver，用于创建配置数据位置解析器的实例, 默认会有两个：
		// ConfigTreeConfigDataLocationResolver和StandardConfigDataLocationResolver
		// ConfigTreeConfigDataLocationResolver: 主要用于解析Config Tree 类型的配置数据位置,Config Tree是SpringBoot2.4.0引入的一种配置数据存储格式
		// StandardConfigDataLocationResolver: 用于解析标准的配置数据位置, 包括application.properties和application.yml
		// 主要使用StandardConfigDataLocationResolver解析配置文件
		...
		// 从spring.factories中拿到ConfigDataLoader，用于创建配置数据加载器的实例, 默认会有两个：
		// ConfigTreeConfigDataLoader和StandardConfigDataLoader
		// ConfigTreeConfigDataLoader: 主要用于加载Config Tree类型的配置数据
		// StandardConfigDataLoader: 用于加载标准的配置数据
		this.loaders = new ConfigDataLoaders(logFactory, bootstrapContext, resourceLoader.getClassLoader());

		// 每个Contributor要么对应一个PropertySource，要么对应一个location（配置文件路径或所在目录）
		// 最后构建的是ConfigDataEnvironmentContributors
	...
	private ConfigDataEnvironmentContributors createContributors(Binder binder) {
		...
		for (PropertySource<?> propertySource : propertySources) {
			if (DefaultPropertiesPropertySource.hasMatchingName(propertySource)) {
				...
				// 将已有的propertySource封装成ConfigDataEnvironmentContributor对象
	...
	private List<ConfigDataEnvironmentContributor> getInitialImportContributors(Binder binder) {
		...
		// SpringBoot将会使用该属性指定的位置作为主要的配置数据来源，并将其导入到应用程序的配置中
		// 调用bindLocations用于对配置数据进行绑定操作
	...
	void processAndApply() {
		...
		// 用来加载和解析配置文件的ConfigDataImporter
	...
	private ConfigDataEnvironmentContributors processInitial(ConfigDataEnvironmentContributors contributors,
		...
		// profile未知的情况下
		// 开始执行加载以及解析配置文件。循环配置数据贡献者直至拿到所有的配置文件

...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		// 根据前面获取的transaction对象判断事务是否已经存在
		// 利用了ThreadLocal判断当前线程是否已经存在事务
		...
		// 如果是PROPAGATION_MANDATORY，则要求当前执行必须有事务，如果没有事务，抛出异常
		...
		// 在当前Thread中没有事务的前提下，以下三个传播机制是等价的
			...
			// 当传入的参数transaction不为null时才会真正的挂起之前的事务
	...
	private TransactionStatus handleExistingTransaction(
		...
		// 如果事务的传播机制是PROPAGATION_NEVER，则抛出异常
		// 因为PROPAGATION_NEVER表示当前执行必须没有事务，如果有事务，则抛出异常
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NEVER) {
			throw new IllegalTransactionStateException(
					"Existing transaction found for transaction marked with propagation 'never'");
		}

		// 如果事务的传播机制是PROPAGATION_NOT_SUPPORTED，则把当前Spring事务挂起，然后不会开启新的事物
		...
		// 如果事务的传播机制是PROPAGATION_REQUIRES_NEW，则把当前事务挂起，然后开启一个新的事务
		...
		// 如果事务的传播机制是PROPAGATION_NESTED，则开启一个嵌套事务
	...
	public final void commit(TransactionStatus status) throws TransactionException {
		...
		// 可以通过TransactionAspectSupport.currentTransactionStatus().setRollbackOnly()方法来设置强制回滚
		...
		// 判断此事务在之前是否设置了全局回滚，跟globalRollbackOnParticipationFailure有关

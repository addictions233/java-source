...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		// TransactionAttribute就是@Transactional注解中的配置
		TransactionAttributeSource tas = getTransactionAttributeSource();
		// 获取@Transactional注解中的属性值
		final TransactionAttribute txAttr = (tas != null ? tas.getTransactionAttribute(method, targetClass) : null);

		// 返回IOC容器中类型为TransactionManager的Bean对象
		// 通常就是DataSourceTransactionManager
		...
		if (txAttr == null || !(ptm instanceof CallbackPreferringPlatformTransactionManager)) {
			...
			// 重点:如果有必要就创建事务，这里就涉及到事务传播机制的实现了
			...
			try {
				...
				// 当前事务已经执行完了, 需要将threadLocal中保存的恢复为oldTransactionInfo
	...
	protected TransactionManager determineTransactionManager(@Nullable TransactionAttribute txAttr) {
		...
		if (StringUtils.hasText(qualifier)) {
			...
			if (defaultTransactionManager == null) {
				...
				if (defaultTransactionManager == null) {
					// 从BeanFactory中获取TransactionManager类型的Bean对象
					// 通常就是DataSourceTransactionManager

...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	public void preInstantiateSingletons() throws BeansException {
		...
		for (String beanName : beanNames) {
			...
			RootBeanDefinition bd = getMergedLocalBeanDefinition(beanName);
			// 非懒加载的, 单例的, 非抽象的BeanDefinition进行初始化
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {
				// 通过BeanType判断需要初始化的Bean是否是FactoryBean
				if (isFactoryBean(beanName)) {
					// 获取FactoryBean对象
					// 通过'&xxxx'获取bean对象, 获得是原始的FactoryBean, 而不是#getObject方法返回的bean
					// 如果原始的FactoryBean还没创建, 就先创建原始的FactoryBean对应的Bean对象
					...
					if (bean instanceof FactoryBean) {
						...
						if (isEagerInit) {
							// FactoryBean创建真正的Bean对象(#getObject方法返回的对象)
							getBean(beanName);
						}
					}
				}
		...
		for (String beanName : beanNames) {
			...
			if (singletonInstance instanceof SmartInitializingSingleton) {
				...
				if (System.getSecurityManager() != null) {
					...
					// k2 生命周期函数调用: 等所有的单例bean都创建之后, 调用该方法
					smartSingleton.afterSingletonsInstantiated();
				}
				smartInitialize.end();
			}
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		if (existingDefinition != null) {
			...
			if (hasBeanCreationStarted()) {
				...
				// 将构建的beanDefinition加入到beanDefinitionMap集合中
				this.beanDefinitionMap.put(beanName, beanDefinition);
				// 将beanName加入到beanDefinitionNames集合中
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			...
			// 如果descriptor所对应的类型是数组、List、Map这些，就将descriptor对应的类型所匹配的所有bean方法，不用进一步做筛选了
			Object multipleBeans = resolveMultipleBeans(descriptor, beanName, autowiredBeanNames, typeConverter);
			if (multipleBeans != null) {
				return multipleBeans;
			}

			// 重点: 找到所有Bean，key是beanName, value有可能是bean对象，有可能是beanClass
			...
			if (matchingBeans.size() > 1) {
				// 重点: 根据类型找到了多个Bean，进一步筛选出某一个, @Primary-->@Priority优先级最高--->name
	...
	protected Map<String, Object> findAutowireCandidates(
		...
		// 从BeanFactory中找出和requiredType所匹配的beanName，仅仅是beanName，这些bean不一定经过了实例化，
		...
		for (String candidate : candidateNames) {
			...
			// isAutowireCandidate是来判断筛选出来的Bean能否用来依赖注入, 包括@Qualifier注解也是这里面处理的
		...
		if (result.isEmpty()) {
			...
			// #isAutowireCandidate()判断找到的bean对象能否用来依赖注入
			// 对泛型依赖注入进行处理
	...
	protected String determineAutowireCandidate(Map<String, Object> candidates, DependencyDescriptor descriptor) {
		...
		// candidates表示根据类型所找到的多个Bean，判断这些Bean中是否有一个是@Primary注解修饰的
		String primaryCandidate = determinePrimaryCandidate(candidates, requiredType);
		if (primaryCandidate != null) {
			return primaryCandidate;
		}

		// 取优先级最高的Bean, 按在@Priority

...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// 马上就要实例化Bean了，确保beanClass被加载了, 使用类加载器加载类
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		synchronized (mbd.postProcessingLock) {
			if (!mbd.postProcessed) {
				try {
					// k2 生命周期函数: 实例化之后,初始化之前执行 BeanDefinition后置处理, 寻找注入点, 寻找@PostConstruct注解
		...
		// 单例bean && 开启循环依赖 && 当前bean正在创建中 ---> 返回true
		...
		if (earlySingletonExposure) {
			...
			// 循环依赖-添加到三级缓存singletonFactories
		...
		// 这一部分代码执行是在初始化之后执行, 即在BeanPostProcessor后置处理器执行之后, 已经进行了AOP生成代理对象
		if (earlySingletonExposure) {
			// 从二级缓存中获取bean
			Object earlySingletonReference = getSingleton(beanName, false);
			if (earlySingletonReference != null) {
				// 二级缓存中的是原始bean对象, 而expostObject中是经过@Async处理过的AOP代理对象, 就不相等, 返回false
				if (exposedObject == bean) {
					exposedObject = earlySingletonReference;
				}
				//allowRawInjectionDespiteWrapping 标注是否允许此 bean 的原始类型被注入到其它 bean 里面，即使自己最终会被包装（生成代理对象）
				// 默认是 false 表示不允许，如果改为 true 表示允许，就不会报错了
				// 另外 hasDependentBean 记录着每个 bean 它所依赖的 bean 的 Map
				else if (!this.allowRawInjectionDespiteWrapping && hasDependentBean(beanName)) {
					// beanName被哪些bean依赖了，现在发现beanName所对应的bean对象发生了改变，违反了单例原则, 那么则会报错
					String[] dependentBeans = getDependentBeans(beanName);
					Set<String> actualDependentBeans = new LinkedHashSet<>(dependentBeans.length);
					for (String dependentBean : dependentBeans) {
						// 循环依赖中dependentBean正在创建中
						// 则removeSingletonIfCreatedForTypeCheckOnly返回false
	...
	protected Object getEarlyBeanReference(String beanName, RootBeanDefinition mbd, Object bean) {
		...
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			for (SmartInstantiationAwareBeanPostProcessor bp : getBeanPostProcessorCache().smartInstantiationAware) {
				// 引入三级缓存，提前对循环依赖的bean在依赖注入的时候就生成代理对象
		...
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			...
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				...
				if (targetType != null) {
					...
					if (bean != null) {
						// 如果返回了Bean, 就要执行初始化之后的生命周期函数, 进行aop等操作
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		// 对于Prototype原型BeanDefinition，会多次来创建Bean，那么就可以把该BeanDefinition所要使用的构造方法缓存起来，避免每次都进行构造方法推断
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		// k2 生命周期函数: 依赖注入DI, 对PropertyValues处理的过程就是依赖注入的处理过程
		...
		if (hasInstAwareBpps) {
			...
			for (InstantiationAwareBeanPostProcessor bp : getBeanPostProcessorCache().instantiationAware) {
				// 这里会调用AutowiredAnnotationBeanPostProcessor#postProcessProperties方法，处理@Autowired注解
				// 调用CommonAnnotationBeanPostProcessor#postProcessProperties方法, 处理@Resource注解
				// BeanPostProcessor内部并不会处理pvs，添加了pvs就直接返回了
	...
	protected void autowireByName(
		...
		for (String propertyName : propertyNames) {
			if (containsBean(propertyName)) {
				// 通过属性名称propertyName作为beanName去获取对应的bean对象
				Object bean = getBean(propertyName);
				// 将上一步找到的结果放入propertyValues中
	...
	protected Object initializeBean(String beanName, Object bean, @Nullable RootBeanDefinition mbd) {
		...
		if (mbd == null || !mbd.isSynthetic()) {
			// k2 生命周期函数: 初始化之前: 调用BeanPostProcessor#postProcessBeforeInitialization
		...
		// 初始化后 AOP
		if (mbd == null || !mbd.isSynthetic()) {
			// k2 生命周期函数: 初始化之后: 调用BeanPostProcessor#postProcessAfterInitialization
	...
	protected void invokeInitMethods(String beanName, Object bean, @Nullable RootBeanDefinition mbd)
		...
		if (isInitializingBean && (mbd == null || !mbd.isExternallyManagedInitMethod("afterPropertiesSet"))) {
			...
			if (System.getSecurityManager() != null) {
				...
				// k2 如果bean对实现了InitializingBean接口, 就调用InitializingBean#afterPropertiesSet生命周期方法
				((InitializingBean) bean).afterPropertiesSet();
			}
		}

...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
		...
		if (sharedInstance != null && args == null) {
			...
			// 如果不是FactoryBean,返回的就是传入的sharedInstance对象
			// 如果sharedInstance是FactoryBean，那么就调用#getObject返回对象
			...
			try {
				...
				// 创建Bean对象需要用到的是合并之后的rootBeanDefinition
				...
				if (mbd.isSingleton()) {
					...
					// bean的生命周期是请求域, 会话域等, 专门处理spring mvc中的一些Bean
		...
		if (beanInstance != null && beanInstance.getClass() != NullBean.class) {
			if (beanInstance instanceof FactoryBean) {
				if (!isFactoryDereference) {
					// 调用factoryBean.getObjectType()返回factoryBean对应的类型type
			...
			if (mbd == null || mbd.stale) {
				...
				if (bd.getParentName() == null) {
					...
					if (bd instanceof RootBeanDefinition) {
						...
						// ScannedGenericBeanDefinition没有继承自RootBeanDefinition
				...
				if (containingBd == null && isCacheBeanMetadata()) {
					// 将新构建的RootBeanDefinition放入到megedBeanDefinitions集合中
	...
	protected Object getObjectForBeanInstance(
		...
		// 判断name是否是'&xxx'，如果是, 那么就直接返回单例池中的对象, 也就是原始的FactoryBean
		if (BeanFactoryUtils.isFactoryDereference(name)) {
		...
		if (mbd != null) {
			...
			// 从factoryBeanObjectCache集合中直接拿对象
			// 所有的factoryBean#getObject方法返回的对象都会放入factoryBeanObjectCache集合中
		...
		if (object == null) {
			...
			// 调用FactoryBean#getObject创建factoryBean真正对象的流程

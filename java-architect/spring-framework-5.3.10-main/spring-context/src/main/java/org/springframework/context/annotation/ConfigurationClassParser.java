...
class ConfigurationClassParser {
			...
			try {
				for (SourceClass candidate : importCandidates) {
					...
					if (candidate.isAssignable(ImportSelector.class)) {
						...
						// 如果import的是DeferredImportSelector，表示延迟处理
						if (selector instanceof DeferredImportSelector) {
							this.deferredImportSelectorHandler.handle(configClass, (DeferredImportSelector) selector);
						} else {

							// 如果import的是普通的ImportSelector, 调用其#selectImports()方法
							String[] importClassNames = selector.selectImports(currentSourceClass.getMetadata());

							// 继续解析ImportSelector#selectImports方法所返回的类
					...
					// 如果import的类实现了ImportBeanDefinitionRegistrar接口
						...
						// 直接实例化ImportBeanDefinitionRegistrar对象
	...
	private class DeferredImportSelectorHandler {
		...
		/**
		 *  对应DeferredImportSelector, 会调用这个方法处理
		 */
		public void process() {
			// 拿到所有的DeferredImportSelectorHolder进行处理
			...
			try {
				if (deferredImports != null) {
					...
					// DeferredImportSelector#getImportGroup方法, 注册所有的Group为BeanDefinition

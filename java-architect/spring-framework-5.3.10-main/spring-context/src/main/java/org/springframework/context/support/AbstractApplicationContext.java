...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		// synchronized锁, startupShutdownMonitor是锁对象
		// 防止#refresh()方法还没执行结束, 又来一个启动或者销毁容器的操作
		synchronized (this.startupShutdownMonitor) {
			StartupStep contextRefresh = this.applicationStartup.start("spring.context.refresh");

			// Prepare this context for refreshing.
			// 提供了#initPropertySources供子类扩展
			prepareRefresh();
			...
			// 2. 添加三个BeanPostProcessor，注意是具体的BeanPostProcessor实例对象,不是BeanDefinition
			// 3. 记录ignoreDependencyInterface
			// 4. 记录ResolvableDependency,在按类型进行依赖注入时可以查找到这个地方记录的ResolvableDependency
			// 5. 添加三个单例Bean, 环境变量Environment涉及的Bean对象
			...
			try {
				...
				// 将扫描到的BeanPostProcessors实例化并排序，并添加到BeanFactory的beanPostProcessors属性集合中去
				// 在Bean对象实例化时会用到各种BeanPostProcessor
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 添加一个BeanPostProcessor，用来处理EnvironmentAware、EmbeddedValueResolverAware等Aware回调
		...
		// 添加resolvableDependencies：在byType按类型进行依赖注入时，会先从这个属性中根据类型找bean
		//  所以找下面四种类型的Bean对象时就会找到后面对应的对象
		...
		// ApplicationListenerDetector负责把ApplicantsListener类型的Bean注册到IOC容器中

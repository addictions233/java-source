...
public abstract class AnnotationConfigUtils {
	...
	public static Set<BeanDefinitionHolder> registerAnnotationConfigProcessors(
		...
		if (beanFactory != null) {
			...
			// 处理@Order注解的AnnotationAwareOrderComparator比较器
			if (!(beanFactory.getDependencyComparator() instanceof AnnotationAwareOrderComparator)) {
				beanFactory.setDependencyComparator(AnnotationAwareOrderComparator.INSTANCE);
			}

			// 用来判断某个Bean能不能用来进行依赖注入
			// 在依赖注入时, 处理@Lazy注解的处理器ContextAnnotationAutowireCandidateResolver
		...
		// 处理@Configuration,  @ComponentScan等注解的配置类BeanFactoryPostProcessor
		...
		// 处理@Autowired注解的BeanPostProcessor
		...
		// 处理@Resource注解的BeanPostProcessor
		...
		// 处理事件监听的BeanFactoryPostProcessor
		...
		// 处理事件监听,创建监听器对象的工厂DefaultEventListenerFactory
		// 注册DefaultEventListenerFactory类型的BeanDefinition，将@EventListener注解修饰的方法封装为ApplicationListener
	...
	private static BeanDefinitionHolder registerPostProcessor(
		...
		registry.registerBeanDefinition(beanName, definition);  // BeanDefinitionMap

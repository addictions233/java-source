...
final class JdkDynamicAopProxy implements AopProxy, InvocationHandler, Serializable {
	...
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		// 调用jdk proxy生成代理对象,
		// 参数this就是JdkDynamicProxy, 它实现了InvocationHandler接口, 所以可以作为参数传入
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		// target被代理对象封装成targetSource
		// 拿到被代理对象
		// this.advised属性就是当前创建代理对象的proxyFactory
		...
		try {
			...
			// advised就是proxyFactory, proxyFactor里面封装了很多的advisor, 要找到与执行方法相匹配的advisor
			...
			if (chain.isEmpty()) {
				...
				// 如果有代理就要执行代理链
				// 重点: 将通知链, 代理对象, 目标对象等封装为MethodInvocation然后调用proceed()方法执行
				// 代理对象在执行某个方法时，根据方法筛选出匹配的Advisor,并适配成Interceptor
			...
			if (setProxyContext) {
				// Restore old proxy.
				// 重新在threadLocal中设置旧的代理对象, 这样就实现出栈和入栈的逻辑

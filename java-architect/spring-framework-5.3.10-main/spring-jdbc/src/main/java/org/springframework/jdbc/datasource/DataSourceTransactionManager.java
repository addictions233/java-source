...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
	...
	protected Object doGetTransaction() {
		...
		// 从当前数据源dataSource对应的Connection连接对象
	...
	protected boolean isExistingTransaction(Object transaction) {
		...
		// 判断connectionHolder中的transactionActive属性值是否为true, 刚创建时默认为false
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			...
			if (!txObject.hasConnectionHolder() ||
				...
				// 获取数据库连接对象Connection，最终都是靠这个Connection连接对象来提交或者回滚事务的
				Connection newCon = obtainDataSource().getConnection();
				if (logger.isDebugEnabled()) {
					logger.debug("Acquired Connection [" + newCon + "] for JDBC transaction");
				}
				// 将新建的数据库连接放入connectionHolder
				txObject.setConnectionHolder(new ConnectionHolder(newCon), true);
			}

			txObject.getConnectionHolder().setSynchronizedWithTransaction(true);
			// 又从connectionHolder中获取数据库连接对象
	...
	protected Object doSuspend(Object transaction) {
		...
		// 解绑数据库连接, resources中移除DataSource对象

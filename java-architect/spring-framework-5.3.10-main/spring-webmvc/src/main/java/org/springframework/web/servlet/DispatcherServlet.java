...
public class DispatcherServlet extends FrameworkServlet {
	...
	private void initHandlerAdapters(ApplicationContext context) {
		...
		if (this.detectAllHandlerAdapters) {
			...
			// 从IOC容器中获取所有的HandlerAdapter类型的bean
		...
		if (this.handlerAdapters == null) {
			// 从DispatcherServlet.properties中获取默认的HandlerAdapter
	...
	protected <T> List<T> getDefaultStrategies(ApplicationContext context, Class<T> strategyInterface) {
		...
		if (value != null) {
			...
			for (String className : classNames) {
				try {
					// 加载类, 也就是DispatcherServlet.properties文件中定义的默认的HandlerMapping
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// 对异步请求的支持, 在Servlet3.0之后才有的(WebFlux)
		...
		try {
			...
			try {
				...
				// 2.进行映射: 查找当前请求对应的handler, 包括handler(控制器)本身和Handler拦截器
				// 返回的是处理器链HandlerExecutionChain
				mappedHandler = getHandler(processedRequest);
				// 未能能找到对应的Handler, 抛出NoHandlerFoundException异常并返回404
				if (mappedHandler == null) {
					noHandlerFound(processedRequest, response);
					return;
				}

				// 3, 找到当前请求对应的最合适的HandlerAdapter
				...
				// 如果没有拦截器返回false, 则表明该拦截器已经处理了返回结果, 直接返回
				...
				// 6. 调用HandlerAdapter中的Handler方法, 真正的开始处理Controller
				// 真正执行我们自己书写的controller层方法, 并返回一个ModelAndView对象
				...
				// 8. 为返回值设置默认视图名: 如果mv有  视图没有，给你设置默认视图
				applyDefaultViewName(processedRequest, mv);

				// 9. 后置拦截器: 执行所有的拦截器中的postHandle方法, 并发mv给它
	...
	private void processDispatchResult(HttpServletRequest request, HttpServletResponse response,
		...
		// 异常视图
		// 执行期间如果存在异常, 通过异常解析器handlerExceptionResolvers来解析异常,返回异常的ModelAndView
		if (exception != null) {
	...
	protected HttpServletRequest checkMultipart(HttpServletRequest request) throws MultipartException {
		// 使用multipartResolver判断是否真正是文件上传请求

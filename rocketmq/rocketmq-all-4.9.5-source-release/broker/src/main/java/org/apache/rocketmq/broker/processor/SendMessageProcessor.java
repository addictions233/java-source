...
public class SendMessageProcessor extends AbstractSendMessageProcessor {
    ...
    private CompletableFuture<RemotingCommand> asyncSendMessage(ChannelHandlerContext ctx, RemotingCommand request,
        ...
        // 发送消息前的处理: 表示RPC的seqNumber,表示队列的读写权限, 自动创建Topic等
        ...
        // 构建Broker内部使用Message (包装了一层)
        MessageExtBrokerInner msgInner = new MessageExtBrokerInner();
        msgInner.setTopic(requestHeader.getTopic());
        msgInner.setQueueId(queueIdInt);
        // 死性队列的处理逻辑: 如果重试次数达到上限,就会进入死性队列
        ...
        if (Boolean.parseBoolean(transFlag)) {
            ...
            // 分布式消息: 事务消息的prepare消息
            putMessageResult = this.brokerController.getTransactionalMessageService().asyncPrepareMessage(msgInner);
        } else {
            // 处理普通消息和事务消息的Commit和Rollback消息

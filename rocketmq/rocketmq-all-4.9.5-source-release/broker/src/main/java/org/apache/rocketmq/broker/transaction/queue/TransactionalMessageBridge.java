...
public class TransactionalMessageBridge {
    ...
    public CompletableFuture<PutMessageResult> asyncPutHalfMessage(MessageExtBrokerInner messageInner) {
        // 重点：parseHalfMessageInner()方法组装事务消息
        return store.asyncPutMessage(parseHalfMessageInner(messageInner));
    }

    /**
     * 事务消息的组装, 事务消息只需要写入CommitLog, 还不写入ConsumeQueue(半消息还不能被消费者消费)
     */
    private MessageExtBrokerInner parseHalfMessageInner(MessageExtBrokerInner msgInner) {
        ...
        // 消息的topic换成了系统自带的半消息topic: RMQ_SYS_TRANS_HALF_TOPIC, 类似于延时消息的套路
        // Rocketmq为事务消息专门准备了这个Topic，消息达到Broker时还是半消息时使用这个Topic覆盖真实的Topic

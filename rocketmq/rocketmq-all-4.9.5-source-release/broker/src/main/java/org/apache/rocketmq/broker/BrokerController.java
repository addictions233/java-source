...
public class BrokerController {
    ...
    public boolean initialize() throws CloneNotSupportedException {
            ...
            try {
                ...
                this.messageStore =
                    new DefaultMessageStore(this.messageStoreConfig, this.brokerStatsManager, this.messageArrivingListener,
                        this.brokerConfig);
                // 如果开启了多副本机制, 会为集群节点选举器添加DLedgerRoleChangeHandler事件处理器
                ...
                // CommitLogDispatcherCalcBitMap是一个消息存储的处理器, 它的作用是计算消息的BitMap信息, 并将其存储到CommitLog中
        ...
        //K2 Broker的Netty组件。注意，Broker即需要是Netty的服务端，又需要是Netty的客户端。
        if (result) {
            // 构建Netty服务端, 监听10911
            this.remotingServer = new NettyRemotingServer(this.nettyServerConfig, this.clientHousekeepingService);
            NettyServerConfig fastConfig = (NettyServerConfig) this.nettyServerConfig.clone();
            // 10909的vipChannelEnabled默认为false
            ...
            // Broker作为Netty服务端,要处理各种Produce和Consumer的请求
            // 为了统一并方案扩展, 就注册了各种Processor, 来应对各种请求
            ...
            //定时进行consumer消费Offset持久化到磁盘的任务, 消费偏移量
            this.scheduledExecutorService.scheduleAtFixedRate(() -> {
                try {
    ...
    /**
     * k1 BrokerController核心的启动方法
     */
    public void start() throws Exception {
        //启动消息存储组件，这里启动服务主要是为了将CommitLog的写入事件分发给ConsumeQueue和IndexFile
        if (this.messageStore != null) {
        ...
        //K2 Broker核心的心跳注册任务，向Namesrv发送心跳包, 需要深入解读下。
        this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {

            @Override
            public void run() {
                try {
                    //同时向所有的NameServer发起心跳注册

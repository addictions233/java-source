...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    /**
     * Producer的核心启动流程就是调用DefaultMQProducerImpl#start()方法
     */
    public void start(final boolean startFactory) throws MQClientException {
        ...
        switch (this.serviceState) {
            ...
            case CREATE_JUST:
                ...
                // k2 重点：获取MQ客户端实例
                // 整个JVM只存在一个MQClientManager实例, 维护一个MQClientInstance缓存表
                // 同一个ClientId只会创建一个MQClientInstance
                // MQClientInstance封装了RocketMQ网络处理API, 是消息生产者和消息消费者与NameServer,Broker打交道的网络通道
                // 命名不规范， 这里其实是获取MQClientInstance实例， 但是取名为mQClientFactory
                this.mQClientFactory = MQClientManager.getInstance().getOrCreateMQClientInstance(this.defaultMQProducer, rpcHook);
                // 注册当前生产者到MQClientInstance管理中, 方便后续网络请求
    ...
    public void checkTransactionState(final String addr, final MessageExt msg,
        ...
        Runnable request = new Runnable() {
            ...
            public void run() {
                ...
                if (transactionCheckListener != null || transactionListener != null) {
                    ...
                    // k2 Producer将上述步骤查到的本地事务状态回调给Broker, 告知Broker半事务消息的最终状态
        ...
        // 默认使用DefaultMQProducerImpl中的checkExecutor线程池来执行检测本地事务状态的任务
    ...
    /**
     * k2 异步发送消息的核心流程: 异步消息不能保证发送消息的有序性，所以发送顺序消息必须同步发送,
     * @param msg
     * @param sendCallback
    ...
    public void send(final Message msg, final SendCallback sendCallback, final long timeout)
        ...
        // 使用的是DefaultMQProducerImpl的asyncSenderExecutor线程池
    ...
    private SendResult sendDefaultImpl(
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            // 重试次数: 同步发送的默认重试次数为2, 所以总共发送次数为3, 而异步发送和单边发送的重试次数为0,所以要可靠发送需要使用同步发送
            ...
            for (; times < timesTotal; times++) {
                ...
                if (mqSelected != null) {
                    ...
                    try {
                        ...
                        // k2 第四步:调用sendKernelImpl进行真正的消息发送
                        sendResult = this.sendKernelImpl(msg, mq, communicationMode, sendCallback, topicPublishInfo, timeout - costTime);
                        // 发送正常: 记录发送结束时间戳
                        // 如果发送消息时长在550ms以内, 则不可用时长为0s
                        // 达到550ms, 不可用时长为30s, 达到1000ms, 不可用时长为60s....
                        endTimestamp = System.currentTimeMillis();
                        // 发送异常: 默认当前的发送时常为30000L
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        // 如果本地缓存的路由消息为空, 则从NameServer中重新获取
        ...
        if (topicPublishInfo.isHaveTopicRouterInfo() || topicPublishInfo.ok()) {
            ...
            // 如果没有找到当前主题的路由消息, 则用默认主题继续去其他NameServer中寻找
    ...
    /**
     * k1 生产者发送顺序消息的方法, 带MessageQueueSelector的发送消息
     */
    private SendResult sendSelectImpl(
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            try {
                ...
                // k2 顺序消息发送通过MessageQueueSelector来获取需要发送的消息队列
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        try {
            // k2 结束事务消息的发送, 会给broker发送消息, 确认事务的提交还是回滚
    ...
    public void endTransaction(
        ...
        // 进行netty调用, Producer告知Broker本地事务的提交状态

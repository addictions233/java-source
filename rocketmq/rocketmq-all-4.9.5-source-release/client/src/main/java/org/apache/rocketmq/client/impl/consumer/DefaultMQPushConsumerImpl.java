...
public class DefaultMQPushConsumerImpl implements MQConsumerInner {
    ...
    public synchronized void start() throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:  // 刚刚创建
                ...
                // 1.检查配置信息
                this.checkConfig();
                // 2.加工订阅信息(如果消息消费为集群模式, 还会为消费者创建一个重试主题)
                ...
                //Consumer负载均衡策略, 默认的负载均衡策略: AllocateMessageQueueAveragely
                this.rebalanceImpl.setAllocateMessageQueueStrategy(this.defaultMQPushConsumer.getAllocateMessageQueueStrategy());
                this.rebalanceImpl.setmQClientFactory(this.mQClientFactory);
                // 拉消息(无论是拉模式, 还是推模式: 数据都是拉取)
                // pullAPIWrapper是拉取消息的API包装类, 主要有消息的拉取方法和接受拉取到的消息
                this.pullAPIWrapper = new PullAPIWrapper(
                    mQClientFactory,
                    this.defaultMQPushConsumer.getConsumerGroup(), isUnitMode());
                this.pullAPIWrapper.registerFilterMessageHook(filterMessageHookList);
                // 消费进度存储,
                // 如果是集群模式, 使用远程存储RemoteBrokerOffsetStore,
                // 如果是广播模式, 则使用本地存储LocalFileOffsetStore,
                if (this.defaultMQPushConsumer.getOffsetStore() != null) {
                    this.offsetStore = this.defaultMQPushConsumer.getOffsetStore();
                } else {
                    switch (this.defaultMQPushConsumer.getMessageModel()) {
                        // 广播模式: 消费进度存储在Consumer本地客户端
                        case BROADCASTING:
                            this.offsetStore = new LocalFileOffsetStore(this.mQClientFactory, this.defaultMQPushConsumer.getConsumerGroup());
                            break;
                            // 集群模式: 消费进度存储在Broker远程服务端
                        case CLUSTERING:
                ...
                // 8.加载消息进度, offsetStore对象是用来操作消费者的消费进度的对象
                // push模式最后消费进度是持久化在broker远程服务端, 但是consumer消费客户端也会在内存中缓存这个消费进度
                ...
                // k2 12. 重点:MQClientInstance启动, 即消费者Consumer开始工作
    ...
    public void persistConsumerOffset() {
        try {
            ...
            // 广播模式消费进度持久化到本地, 集群消费模式消费进度持久化到远程Broker

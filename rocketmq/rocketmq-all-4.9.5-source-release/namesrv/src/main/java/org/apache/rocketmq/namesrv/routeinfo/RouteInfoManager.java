...
public class RouteInfoManager {
    ...
    // 读写锁, 控制各个Map在并发读写下的安全性
    private final ReadWriteLock lock = new ReentrantReadWriteLock();
    // topic对应队列信息
    private final HashMap<String/* topic */, Map<String /* brokerName */ , QueueData>> topicQueueTable;
    // broker信息, key-broker名称, value-对应的broker的数据信息
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {//加锁，同一时间只能一个线程写
                this.lock.writeLock().lockInterruptibly();
                // clusterAddrTable: broker集群信息, key-集群名称, value-对应集群中所有broker名称
                Set<String> brokerNames = this.clusterAddrTable.computeIfAbsent(clusterName, k -> new HashSet<>());
                brokerNames.add(brokerName);

                boolean registerFirst = false;
                //brokerAddrTable: broker路由信息表,  key-broker名称, value-对应的broker的数据信息
                ...
                // brokerAddrsMap: broker地址信息, key-brokerId, value-对应的broker地址
                // 非第一次创建, 则更新brokerData
    ...
    public int scanNotActiveBroker() {
        ...
        while (it.hasNext()) {
            ...
            // 如果收到心跳包的时间距离当前时间是否超过broker心跳超时时间

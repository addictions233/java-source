...
public class CommitLog {
    ...
    public CommitLog(final DefaultMessageStore defaultMessageStore) {
        ...
        if (FlushDiskType.SYNC_FLUSH == defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            ...
            // 同步刷盘
            // FlushRealTimeService线程默认每500ms将MappedByteBuffer中新追加的内存写入到硬盘
        ...
        // useReentrantLockWhenPutMessage参数默认为false,使用自旋锁
        // 异步刷盘机制建议使用自旋锁AtomicInteger,
        // 同步刷盘机制建议使用重入锁ReentrantLock, 同步刷盘竞争比较激烈,
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessage(final MessageExtBrokerInner msg) {
        // Set the storage time  Message存储到Broker的时间
        msg.setStoreTimestamp(System.currentTimeMillis());
        // Set the message body BODY CRC (consider the most appropriate setting
        // on the client)
        // Message Body的crc验证码,防止消息内容被篡改和破坏
        msg.setBodyCRC(UtilAll.crc32(msg.getBody()));
        // Back to Results
        AppendMessageResult result = null;
        // 存储消息的耗时指标, 可以收集这些指标发送到监控系统
...
//        int queueId msg.getQueueId();
        ...
        if (tranType == MessageSysFlag.TRANSACTION_NOT_TYPE
            ...
            // Delay Delivery
            // K1 处理延时消息:延迟消息转到系统Topic
            // 延时消息的处理逻辑就是先将这些消息包装一层放入Rocketmq自己的延迟Topic消息队列中
            // 等到了延时消息对应的延时时间, 就将这些消息投递到它原本的消息队列中
            if (msg.getDelayTimeLevel() > 0) {
                if (msg.getDelayTimeLevel() > this.defaultMessageStore.getScheduleMessageService().getMaxDelayLevel()) {
                    msg.setDelayTimeLevel(this.defaultMessageStore.getScheduleMessageService().getMaxDelayLevel());
                }
                // Rocketmq系统自定义的延迟Topic
                topic = TopicValidator.RMQ_SYS_SCHEDULE_TOPIC;
                // 该延迟Topic下对于18个不同的延时级别有不同的队列queue
        ...
        try {
            ...
            // global
            // 拿到锁之后再设置一次, 这样可以做到全局有序
            msg.setStoreTimestamp(beginLockTimestamp);
            // 如果最近的一个CommitLog文件满了, 就创建一个新的
    ...
    public CompletableFuture<PutMessageStatus> submitReplicaRequest(AppendMessageResult result, MessageExt messageExt) {
        if (BrokerRole.SYNC_MASTER == this.defaultMessageStore.getMessageStoreConfig().getBrokerRole()) {
            // 核心通过HAService实现主从同步, 使用Java NIO
    ...
    class GroupCommitService extends FlushCommitLogService {
        ...
        /**
         * 读写队列互换, 防止读的时候存在并发的写, 这样存在并发安全问题
         */
            ...
            if (!this.requestsRead.isEmpty()) {
                for (GroupCommitRequest req : this.requestsRead) {
                    ...
                    for (int i = 0; i < 2 && !flushOK; i++) {
                        // 刷盘
                        // 底层最终会调用MappedByteBuffer#force方法真正的将Buffer数据落盘

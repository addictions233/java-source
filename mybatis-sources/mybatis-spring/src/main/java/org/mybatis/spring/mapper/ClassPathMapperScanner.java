...
public class ClassPathMapperScanner extends ClassPathBeanDefinitionScanner {
  ...
  public Set<BeanDefinitionHolder> doScan(String... basePackages) {
    ...
    if (beanDefinitions.isEmpty()) {
      ...
      /**
       * 正是在这里mybatis做了一个很牛逼的功能，将spring的 的bean定义玩到极致(做了偷天换日的操作) 现在我们知道t通过父类扫描出来的mapper是接口类型的
  ...
  private void processBeanDefinitions(Set<BeanDefinitionHolder> beanDefinitions) {
    ...
    for (BeanDefinitionHolder holder : beanDefinitions) {
       ...
       * 的字段属性是永远自动注入不了值的因为字段上是没有 @AutoWired注解,所以我们需要把UserMapper<MapperFactoryBean> 的bean定义的注入模型给改成我们的 AUTOWIRE_BY_TYPE= 1,
       * 指定这个类型就是根据类型装配的话， 第一:我们的字段上不需要写@AutoWired注解，为啥? springioc会把当前UserMapper<MapperFactoryBean>中的setXXX(入参)

...
public class DistroConsistencyServiceImpl implements EphemeralConsistencyService, DistroDataProcessor {
    ...
    /**
     * k2 在该对象初始化完成之后就启动消费实例注册的异步任务
     */
    @PostConstruct
    public void init() {
        // 使用异步线程池去实例注册的任务队列中获取任务并执行
        GlobalExecutor.submitDistroNotifyTask(notifier);
    }

    @Override
    public void put(String key, Record value) throws NacosException {
        // k2 重点:核心的服务注册逻辑 在#onPut()方法中
        onPut(key, value);
        // 集群模式下,客户端新增注册实例, 在服务端实例间同步的过程
    ...
    public void onPut(String key, Record value) {
        ...
        // 通过传入的DataOperation任务类型, 判断是新增, 修改, 删除, 确认
    ...
    public class Notifier implements Runnable {
        ...
        /**
         * k1 异步线程实现服务实例的真正注册, 所有的任务都放在了一个阻塞队列中, 启动一个定时任务去消费阻塞队列中的任务
         */
        @Override
        public void run() {
            Loggers.DISTRO.info("distro notifier started");

            // 这里是一个死循环, 一直有个一个线程不断地去注册服务实例的任务阻塞队列中获取任务并执行
            for (; ; ) {
                try {
                    // 从阻塞队列中获取需要处理的服务实例

...
public class ServiceManager implements RecordListener<Service> {
    ...
    /**
     * k2 nacos服务注册表: key-namespace key-serviceName, value-service
     * 注册实例是 NameSpace --> Group ---> Service --> Cluster --> Instance的结构
    ...
    public void createServiceIfAbsent(String namespaceId, String serviceName, boolean local, Cluster cluster)
        ...
        if (service == null) {
            ...
            // 构建服务注册表结构: Namespace --> Group --> Service ---> Cluster ----> Instance
            service = new Service();
            ...
            // 将构建的service放入注册实例表serviceMap中, 并初始化健康检测任务
            putServiceAndInit(service);
            if (!local) {
    ...
    public void registerInstance(String namespaceId, String serviceName, Instance instance) throws NacosException {
        // 从注册列表serviceMap获取改service实例, 如果没有就创建一个空的service实例, 然后放入注册列表serviceMap
        // 因为该service实例还没有放入instance, 所有叫空的service
        createEmptyService(namespaceId, serviceName, instance.isEphemeral());

        // 上一步放入了, 再从注册列表serviceMap获取改service实例
        Service service = getService(namespaceId, serviceName);
    ...
    public void addInstance(String namespaceId, String serviceName, boolean ephemeral, Instance... ips)
        ...
        // 从serviceMap中获取空的service结构
        Service service = getService(namespaceId, serviceName);

        synchronized (service) {
            // 新增实例并返回所有的实例, 使用写时复制copy on write的思想。
            List<Instance> instanceList = addIpAddresses(service, ephemeral, ips);

            Instances instances = new Instances();
            instances.setInstanceList(instanceList);
            // 重点: 真正的使用异步线程添加实例, 替换原有的实例列表
    ...
    public List<Instance> updateIpAddresses(Service service, String action, boolean ephemeral, Instance... ips)
            throws NacosException {
        // 通过命名空间namespaceId和服务名称service name获取旧的实例集合
    ...
    /**
     * 将新注册的实例Instance添加对应服务Service的实例列表List<Instance>中
     */
    ...
    /**
     * k1 集群模式下, 客户端注册实例的健康状态在多个服务端实例中同步
     */

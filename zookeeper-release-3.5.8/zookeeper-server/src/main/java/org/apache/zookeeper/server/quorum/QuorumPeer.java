...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public synchronized void start() {
        ...
        try {
            // 启动JettyServer, 8080端口访问, 通常用于运维查看zookeeper的一些基础信息
            adminServer.start();
        } catch (AdminServerException e) {
            // 启动不了JettyServer也不会有问题, 这里直接捕获了异常
    ...
    synchronized public void startLeaderElection() {
       try {
           // 判断当前节点状态是不是LOOKING, 当没有选举出Leader时, 所有的节点状态都是LOOKING
           // 节点总共有四种状态: LOOKING, FOLLOWING, LEADING, OBSERVING
           if (getPeerState() == ServerState.LOOKING) {
               // 构建当前节点的投票信息: 由myId, 最大的Zxid和当前的Epoch(当前选举的轮数)
      ...
      //      throw new RuntimeException("My id " + myid + " not in the peer list");
        //}
        // QuorumPeer初始时的electionType值为3, 所以这里不相等
        if (electionType == 0) {
            try {
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm){
        Election le=null;
        // zookeeper的leader选举有多种算法可供选择, 对应不同的electionAlgorithm值
        // 我们使用electionAlgorithm=3, 其他的都过时了
        ...
        case 3:
            ...
            if(listener != null){
                // listener继承了ZooKeeperThread类, 所以就是一个线程, 调用#start方法就是执行#run方法
                listener.start();
                // 快速Leader选举算法
    ...
    public void run() {
        ...
        try {
            ...
            while (running) {
                switch (getPeerState()) {
                case LOOKING:  // 没有Leader时是LOOKING状态
                    LOG.info("LOOKING");
